...
The branch also sets `$esp_apple`, which the memory-tester entry reads to pick a video mode that firmware actually has — see *The memory tester* below. And `$esp_vendor` is left set whatever the manufacturer turns out to be, so a disk can key other per-machine choices off it from `custom.cfg`, which is sourced last but still before the menu is drawn:
...
**Apple firmware is the exception, and gets `keep` outright.** A Mac offers exactly one video mode — GRUB's own `videoinfo` on an iMac14,2 lists `2560x1440x32` from the EFI GOP driver and nothing else — so there is no small mode to ask for, and asking for one anyway leaves the panel dark with the tester running invisibly behind it, live enough to reboot on ESC. The entry reads `$esp_apple`, which the master sets when it recognises the manufacturer, so machines that do have a 640x480 mode still get it. The cost on a Mac is that the 640x400 panel sits in the middle of a 2560x1440 screen.
