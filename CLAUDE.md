...
4. **Translate identifiers** — `rewrite_fstab()` rewrites old→new UUIDs, retargets the existing UUID-based swap entry to `NEW_UUID_SWAP` in place (appending one only if none exists, and dropping stale disabled swap lines), and comments out foreign mounts / binds (tagged `# [PORTABLE-SYNC-DISABLED]`, never re-prefixed on later syncs, keeping only the `/tmp → /var/tmp` bind); `rewrite_grub_distributor()` stamps the target disk model (or the `--brand` override) into the GRUB menu branding.
